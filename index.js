...
module.exports = async (phone, code) => {
  code = code.trim()
  const matched = code.match(/(?<=\?v=)([0-9A-Za-z])+/)
  code = matched ? matched[0] : code
  if (!code || code.length < 18) {
    throw new Error('INVAILD_VOUCHER_CODE')
  }
  const res = await f(`https://gift.truemoney.com/campaign/vouchers/${code}/redeem`, 'POST').body({ mobile: phone, voucher_hash: code }).json()
